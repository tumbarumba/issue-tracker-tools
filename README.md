...
Usage: cfd [OPTIONS]
  ...
  Use `--csv` or `--excel` options to generate a cdf directly from a progress
  report.

  Use `--project` or `--epic` options if creating a cfd from the output of the
  `it project` command (Jira will need configured).

Options:
  -t, --today [%Y-%m-%d]          Override today's date
  -d, --days INTEGER              Number of days to calculate trends (default:
                                  14)
  -p, --project-label, --project TEXT
                                  Project label
  -e, --epic TEXT                 Epic key
  -s, --csv PATH                  CSV file
  -x, --excel PATH                Excel file
  -f, --show-first-date           Show first date in report data
  -l, --show-last-date            Show last date in report data
  -c, --config PATH
  -o, --open-graph                Open the graph after generation
  -v, --verbose                   Show extra information from report
  -h, --help                      Show this message and exit.
```

To see this in action, look in [example/cfd](example/cfd) for example progress reports that can be used to generate a cfd.

For example:

```shell
cfd --today 2026-05-12 --csv example/cfd/consistent/consistent-progress.csv
```
